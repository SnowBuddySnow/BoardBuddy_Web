...
const senderLabel = (item: NotificationItem) => item.senderType === 'MANAGER' ? 'Crew manager' : item.senderType === 'DEVELOPER' ? 'BoardBuddy' : 'System';

export default function Notifications({ onBack, onChanged }: Props) {
    const [feed, setFeed] = useState<NotificationFeed | null>(null);
    const [error, setError] = useState('');

    useEffect(() => { getNotifications().then(setFeed).catch(() => setError('Could not load notifications.')); }, []);
    ...
    return (
        <main className="flex h-full min-h-0 flex-col bg-[#FAF8F3] dark:bg-zinc-950">
            <header className="flex items-center gap-3 border-b border-zinc-200 px-4 py-4 dark:border-zinc-800">
                <button onClick={onBack} aria-label="Back" className="rounded-full p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800"><ArrowLeft /></button>
                <div><h1 className="text-xl font-bold">Notifications</h1><p className="text-xs text-zinc-500">{feed?.unreadCount ?? 0} unread</p></div>
            </header>
            <section className="min-h-0 flex-1 overflow-y-auto p-4">
                ...
                {feed?.items.length === 0 && <div className="py-20 text-center text-zinc-500"><Bell className="mx-auto mb-3"/><p>No notifications yet.</p></div>}
